...
def parse_params(params) -> dict:
    """Safely parse parameters from string or dict"""
    ...
    raise ValueError(f"Params must be string or dict, got {type(params)}")
...
def analyze_image_tool(params: str) -> str:
    ...
    try:
        ...
        if response.status_code != 200:
            raise Exception(f"Script generation failed: {response.status_code} - {response.text}")
...
def generate_audio_tool(params: str) -> str:
    """
    Generates audio using ElevenLabs TTS via AIML API.
    Params: {"script_text": str, "voice_settings": dict}
    voice_settings format: {
    ...
    try:
        ...
        voice_config = params_dict.get("voice_settings", {})
        
        if not script_text or not script_text.strip():
            raise ValueError("script_text cannot be empty")
        
        # Get ElevenLabs settings
        voice_model = voice_config.get("model", "eleven_turbo_v2_5")
        voice_name = voice_config.get("voice", "Rachel")
        
        # Add elevenlabs/ prefix if not present
        if not voice_model.startswith("elevenlabs/"):
            voice_model = f"elevenlabs/{voice_model}"
        ...
        if "voice_settings" in voice_config:
            payload["voice_settings"] = voice_config["voice_settings"]
        
        print(f"Generating audio with ElevenLabs model: {voice_model}, voice: {voice_name}")
        
        response = requests.post(
            f"{AIML_BASE_URL}/v1/tts",
            headers=headers,
            json=payload,
            timeout=120,
            stream=True
        ...
        # Save the audio file directly from stream
...
def generate_video_clip_tool(params: str) -> str:
    ...
    Params: {"image_path": str, "prompt": str, "clip_index": int}
    ...
    try:
        ...
        print(f"Clip {clip_index}: Submitting to Veo 3.1...")
        ...
        error_msg = f"Video clip generation error: {str(e)}"
        print(error_msg)
        raise Exception(error_msg)
...
def assemble_video_tool(params: str) -> str:
    ...
    try:
        ...
        error_msg = f"Video assembly error: {str(e)}"
        print(error_msg)
        raise Exception(error_msg)
...
def apply_lip_sync_tool(params: str) -> str:
    ...
    try:
        ...
        error_msg = f"Lip sync error: {str(e)}"
        print(error_msg)
        raise Exception(error_msg)